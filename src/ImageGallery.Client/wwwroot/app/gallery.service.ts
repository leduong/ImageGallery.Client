...
export class GalleryService {
  ...
  public getGalleryIndexViewModel(limit: number, page: number) {
    ...
    return new Promise((resolve, reject) => {
      this.httpClient
        .get(`${this.baseUrl}/list?limit=${limit}&page=${page}`, {
          observe: 'response',
          headers: self.generateBearerHeaaders(),
        })
        .subscribe(
          res => {
            resolve({
  ...
  public getAlbumIndexViewModel(limit: number, page: number) {
    ...
    return new Promise((resolve, reject) => {
      this.httpClient
        .get(`${this.albumUrl}/list?limit=${limit}&page=${page}`, {
          observe: 'response',
          headers: headers,
        })
        .subscribe(
  ...
  public getAlbumViewModel(
    id: string,
    limit: number,
    page: number
  ): Observable<IAlbumViewModel> {
    var headers = this.generateBearerHeaaders()
    headers.append('Content-Type', 'application/json')

    return this.httpClient
      .get<IAlbumViewModel>(
        `${this.albumUrl}/images/list/${limit}/${page}?id=${id}`,
        { headers: this.generateBearerHeaaders() }
      )
  ...
  public getEditImageViewModel(id: string): Observable<IEditImageViewModel> {
    var self = this
    return this.httpClient
      .get<IEditImageViewModel>(`${this.baseUrl}/${id}`, {
        headers: self.generateBearerHeaaders(),
      })
      .catch(this.handleError)
  }
  ...
  public getPhotoAttraction(id: string): Observable<Object> {
    return this.httpClient
      .get(`${this.photoUrl}/api/photo/attraction/${id}`, {
        headers: this.generateBearerHeaaders(),
      })
      .catch(this.handleError)
  }

  public postEditImageViewModel(
    model: IEditImageViewModel
  ): Observable<Object> {
    var headers = this.generateBearerHeaaders()
    headers.append('Content-Type', 'application/json')

    return this.httpClient.post(`${this.baseUrl}/edit`, model, {
      headers: headers,
    })
  }
  ...
  public postImageViewModel(model: IAddImageViewModel): Observable<Object> {
    ...
    return this.httpClient
      .post(`${this.baseUrl}/order`, formData, options)
      .catch(this.handleError)
  }
