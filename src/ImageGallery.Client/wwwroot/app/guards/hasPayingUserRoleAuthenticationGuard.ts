...
export class HasPayingUserRoleAuthenticationGuard implements CanActivate {
  ...
  canActivate(): boolean {
    ...
    console.log(
      `[HasPayingUserRoleAuthenticationGuard] -> [OidcSecurityService] -> [getIsAuthorized] raised with ${
        this.isAuthorized
      }`
    )
    ...
    console.log(
      `[HasPayingUserRoleAuthenticationGuard] -> [OidcSecurityService] -> [getUserData] raised`
    )
