...
export class AlbumComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.authService.getIsAuthorized().subscribe((isAuthorized: boolean) => {
      if (isAuthorized) {
        this.pagination.limit = localStorage.getItem('album-limit')
          ? parseInt(localStorage.getItem('album-limit'))
          : 15
        this.pagination.page = localStorage.getItem('album-page')
          ? parseInt(localStorage.getItem('album-page'))
          : 1
  ...
  private getAlbumIndexViewModel(event?) {
    ...
    this.galleryService
      .getAlbumIndexViewModel(this.pagination.limit, this.pagination.page)
      .then((response: any) => {
