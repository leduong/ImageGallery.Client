...
export class GalleryComponent implements OnInit {
  ...
  constructor(
    ...
    this.savedAlbums = JSON.parse(localStorage.getItem('albums'))
  ...
  ngOnInit() {
    ...
    this.authService.getIsAuthorized().subscribe((isAuthorized: boolean) => {
      if (isAuthorized) {
        this.pagination.limit = localStorage.getItem('limit')
          ? parseInt(localStorage.getItem('limit'))
          : 15
        this.pagination.page = localStorage.getItem('page')
  ...
  ngAfterViewInit() {
    if (localStorage.getItem('isEdited') == 'yes') {
      ...
      this.toastr.success('Image has been edited successfully!', 'Success!', {
        closeButton: true,
      })
      localStorage.removeItem('isEdited')
    } else if (localStorage.getItem('isAdded') == 'yes') {
      this.toastr.success('Image has been added successfully!', 'Success!', {
        closeButton: true,
      })
  ...
  public deleteImage(imageId: string) {
    this.galleryService.deleteImageViewModel(imageId).subscribe(
      response => {},
      (err: any) => {
        this.toastr.error('Failed to delete image!', 'Oops!', {
          closeButton: true,
        })
        console.log(err)
      },
      () => {
        ...
        this.galleryIndexViewModel.images = this.galleryIndexViewModel.images.filter(
          x => x.id != imageId
        )
      }
    )
  ...
  private getGalleryIndexViewModel(event?) {
    ...
    this.galleryService
      .getGalleryIndexViewModel(this.pagination.limit, this.pagination.page)
      .then((response: any) => {
        ...
        this.galleryIndexViewModel.images.forEach((image, i) => {
          this.albums[i] =
            this.savedAlbums.findIndex(album => album.id === image.id) > -1
        })
  ...
  public showModal(photo, template) {
    this.spinnerService.show()
    this.galleryService
      .getPhotoAttraction(photo.photoId)
      .subscribe((res: any) => {
        this.flickrList = res
