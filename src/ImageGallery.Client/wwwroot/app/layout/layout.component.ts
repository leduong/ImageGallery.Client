...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.isAuthorizedSubscription = this.authService
      .getIsAuthorized()
      .subscribe((isAuthorized: boolean) => {
        console.log(
          `[AuthService] -> [getIsAuthorized] raised with ${isAuthorized}`
        )

        this.isAuthorized = isAuthorized
      })

    this.isUserInRoleSubscription = this.authService
      .checkUserRole(RolesConstants.PayingUser)
      .subscribe((isInRole: boolean) => {
